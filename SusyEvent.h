...
// $Id: SusyEvent.h,v 1.4 2013/06/20 19:51:18 weinberg Exp $
//
...
namespace susy {
  ...
  public:
    ...
    float dzPv (TVector3 const& pv) const { return vertex.Z() - pv.Z() - ((vertex.X() - pv.X()) * momentum.Px() + (vertex.Y() - pv.Y()) * momentum.Py()) / momentum.Pt() * (momentum.Pz() / momentum.Pt()); }
    float dxyPv(TVector3 const& pv) const { return ((-vertex.X() + pv.X()) * momentum.Py() + (vertex.Y() - pv.Y()) * momentum.Px()) / momentum.Pt(); }
