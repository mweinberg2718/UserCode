...
//   chain.Add("dcap:///pnfs/cms/WAX/11/store/user/lpcsusystealth/ntuples/stealthT2_gg/susyEvents_165_1_LBp.root");
  ...
  std::cout << "Total events in chain (should equal total events below): " << chain.GetEntries() << std::endl;
...
//   sea.AddHltName("HLT_Photon36_CaloId10_Iso50_Photon22_CaloId10_Iso50");
  ...
  sea.SetDatasetName("stealthT2_gg");  // Currently for event weighting; can be removed for non-signal
  sea.SetCutComplement("jetHt");  // Possible: singleMu, singleElectron, doublePhoton, or photonHad, jetHt (default)
